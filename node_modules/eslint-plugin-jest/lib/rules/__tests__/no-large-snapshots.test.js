...
const _require2 = require('babel-eslint'),
...
describe('no-large-snapshots', () => {
  ...
  describe('ExpressionStatement function', () => {
    ...
    it('should throw exeption if relative paths are passed as whitelist keys', () => {
      ...
      expect(() => noLargeSnapshots(mockContext).ExpressionStatement(snapshotNode)).toThrow('All paths for whitelistedSnapshots must be absolute. You can use JS config and `path.resolve`');
    });
    it('should not throw exeption if absolute paths are passed as whitelist keys', () => {
      ...
      expect(() => noLargeSnapshots(mockContext).ExpressionStatement(snapshotNode)).not.toThrow();
    });
  });
});